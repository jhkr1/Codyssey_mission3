...
def make_result(case_id, expected, prediction, score_cross, score_x, passed, reason=""):
    """패턴 한 건의 분석 결과를 같은 모양의 딕셔너리로 만든다."""
    return {
        "case_id": case_id,
        "expected": expected,
...
def extract_size_from_key(case_id):
...
def sort_filter_key(filter_key):
...
def analyze_pattern_case(case_id, case_payload, filters_by_size):
    """data.json의 패턴 한 건을 검증하고 Cross/X 판정을 수행한다.

    케이스 키에서 크기를 찾고, expected 라벨을 표준화하고, 입력 행렬 크기를 확인한다.
    문제가 있으면 프로그램을 멈추지 않고 결과 딕셔너리에 실패 이유를 담아 반환한다.
    """
    if not isinstance(case_payload, dict):
        return make_result(case_id, None, "ERROR", None, None, False, "케이스 구조가 객체가 아닙니다.")

    size = extract_size_from_key(case_id)
    if size is None:
        return make_result(case_id, None, "ERROR", None, None, False, "케이스 키가 size_N_idx 형식이 아닙니다.")

    if size not in filters_by_size:
        return make_result(case_id, None, "ERROR", None, None, False, f"size_{size} 필터를 찾을 수 없습니다.")

    expected = normalize_label(case_payload.get("expected"))
    if expected is None:
        raw_expected = case_payload.get("expected")
        return make_result(case_id, None, "ERROR", None, None, False, f"expected 라벨을 정규화할 수 없습니다: {raw_expected}")

    try:
        pattern = validate_matrix(case_payload.get("input"), expected_size=size, name=f"{case_id}.input")
    except ValueError as error:
        return make_result(case_id, expected, "ERROR", None, None, False, str(error))
    ...
    if prediction == expected:
        return make_result(case_id, expected, prediction, score_cross, score_x, True)
    ...
    return make_result(case_id, expected, prediction, score_cross, score_x, False, reason)
